...
## js垃圾回收机制
1.首先垃圾回收是因为有些变量在内存中没有被使用所以需要垃圾回收机制来进行回收<br>
2.如果在内存中的变量没有被回收并且也没有被使用就会造成内存泄漏
3.垃圾回收机制有两种方式一种是标记清除一种是引用计数法

### 标记清除
1.当产生一个变量的时候就会进行标记当启动垃圾回收的时候就会把标记的变量进行检查如果没有使用就进行回收

### 引用计数
1.当一个变量进入一个作用域之后会进行标记当这个作用域销毁之后被标记的变量就被清除了作用域一般就是说一个函数的局本行为

## instanceof的原理
1.[] instanceof Array<br>
2.在函数中会重复遍历左边值的原型对象是否等于右面值的prototype<br>
3.查找到相等的时候直接返回true<br>
4.最后查找完之后如果没有相等的就返回false